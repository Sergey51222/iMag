(() => {
	...
	function isWebp() {
		...
		// Add _webp or _no-webp class for HTML
		testWebP((function (support) {
			let className = support === true ? "webp" : "no-webp";
			document.documentElement.classList.add(className);
		}));
	}
	// Get the hash on the site address

	function getHash() {
		if (location.hash) return location.hash.replace("#", "");
	}
	// A hash pointer to the site address
	...
	// Helper modules to lock scroll and jump
	...
	function spollers() {
		...
		if (spollersArray.length > 0) {
			// Getting regular slayers
			const spollersRegular = Array.from(spollersArray).filter((function (item, index, self) {
				return !item.dataset.spollers.split(",")[0];
			}));
			// Initialization of normal layers
			if (spollersRegular.length) initSpollers(spollersRegular);
			// Getting layerers with media requests
	...
	function dataMediaQueries(array, dataSetValue) {
		// Get objects with media requests
		const media = Array.from(array).filter((function (item, index, self) {
			if (item.dataset[dataSetValue]) return item.dataset[dataSetValue].split(",")[0];
		}));
		// Initialize objects with media requests
		if (media.length) {
	...
	function initSliders() {
		// The first slider on the main page. In the "Popular products" tab.
		...
		// The second slider on the main page. In the "Promotional offers" tab.
		if (document.querySelector(".promotion-swiper")) new core(".promotion-swiper", {
			modules: [Navigation, Pagination, Autoplay],
			observer: true,
			observeParents: true,
			slidesPerView: 1,
		...
		// The third slider on the main page. In the "News" tab.
		if (document.querySelector(".novelties-swiper")) new core(".novelties-swiper", {
			modules: [Navigation, Pagination, Autoplay],
			observer: true,
			observeParents: true,
			slidesPerView: 1,
		...
		// The fourth slider on the main page. In the "Soon to be sold" tab.
		if (document.querySelector(".soon-swiper")) new core(".soon-swiper", {
			modules: [Navigation, Pagination, Autoplay],
			observer: true,
			observeParents: true,
			slidesPerView: 1,
		...
		// The fifth slider on the Home page. Block "News"
		...
		// Swiper on the Card page. Block of products.
		...
		// Swiper on the Card page. "This product is often bought" block
